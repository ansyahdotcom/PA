	<header class="bg-warning py-5 mb-5">
		<div class="container h-100">
			<div class="row h-100 align-items-center">
				<a href="<?= base_url('class');?>" class="btn button secondary-button float-left"><i class="fas fa-arrow-left"></i> Kembali</a>
				<?php foreach ($kelas as $row) { ?>
					<div class="col-lg-12">
						<div class="site-buttons p-5">
					<div class="d-flex flex-row flex-wrap">
						<img src="<?= base_url('assets/dist/img/kelas/'). $row->GBR_KLS;?>" width="300" alt="gambar kelas" class="img-fluid ml-5">
						<div class="kelas ml-5">
							<h4 class="display-4 title-text"><?= $row->TITTLE; ?></h4>
							<p class="para text-dark">Oleh : <?= $row->NM_ADM?></p>
							<p class="badge bg-primary p-2 text-white">Kategori: <?= $row->KTGKLS?></p>
							<h5 class="font-weight-bold">Rp. <?= number_format($row->PRICE, 0, ".", "."); ?></h5>
							<a href="<?= base_url('register')?>" class="btn button primary-button mt-5 float-right"><i class="fas fa-shopping-cart"></i> Beli Kelas</a>
						</div>
					</div>
				</div>
			</div>
	...
	<div class="container mt-2 mb-2">
		<div class="card">
			...
			<div class="card-body">
                ...
                <p class="font-weight-bold"><?= 'Mulai : '. date('d F Y', strtotime($row->TGL_MULAI)); ?></p>
                <p class="font-weight-bold"><?= 'Selesai : '. date('d F Y', strtotime($row->TGL_SELESAI)); ?></p>
			</div>
		</div>
	</div>
